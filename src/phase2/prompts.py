...
6. TRUST THE ANALYSIS.
   The Phase 1 analysis is your single source of truth. It was generated by a deep code analysis agent that read and verified the codebase directly. Trust its findings. If a specific detail (e.g., a configurable value, threshold, or error handling behavior) is not mentioned in the analysis, mark it as [TBC — To Be Confirmed] rather than guessing.
...
8. OUTPUT FORMAT.
   ...
   CRITICAL: bullet_list and numbered_list "items" MUST be plain strings only — never nested JSON objects.
   WRONG: {{"type": "bullet_list", "items": [{{"type": "paragraph", "text": "..."}}]}}
   CORRECT: {{"type": "bullet_list", "items": ["...", "...]}}.
   CRITICAL: table and decision_table "rows" MUST be a list of lists: [["val1","val2",...], ...] — never a list of dicts.
   WRONG: {{"type": "table", "rows": [{{"Column A": "val1", "Column B": "val2"}}]}}
   WRONG: {{"type": "table", "rows": [{{"cells": ["val1","val2"]}}]}}
   CORRECT: {{"type": "table", "rows": [["val1", "val2"]]}}.
   decision_table rows must also be lists: [["condition1", "condition2", "action"], ...]
